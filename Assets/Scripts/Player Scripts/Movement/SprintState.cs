...
public class SprintState : MovementState
{
    ...
    {
        if (!GAME.Instance.player.isGrounded || GAME.Instance.player.isGrappling) return;
    ...
    private void playerSprint(MovementController controller)
    {
        ...
        controller.playerController.velocity = relativeDirection * GAME.Instance.player.sprintSpeed;
    }
}